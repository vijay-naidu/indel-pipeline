# James Stimson, 17th December 2018
# based on code from writefastaprjeb.py

#TO DO: documenst main lists and dicts

#TO DO: optionally remove hypers

# check against manually-crafted results: Wont be the same if trimmed
...
for label in sorted(name_list):
    ...
    file1 = open(vcf_path + 'snps' + label + '.recode.vcf')
    list0 = listfromvcf(file1)  # list of indel calls 
    #break # for TEST
    name_count+=1
    local_list = []
    print(label)
...
matrix_file_name = "total_matrix_" + str(DEP_CUTOFF) + ".csv"
